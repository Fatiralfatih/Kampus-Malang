<x-pengunjung-layout>
    @if (!request('search'))
        ...
        <section class="pt-11 dark:bg-gray-950 dark:text-white">
            <div class="container bg-base-200 rounded-lg dark:bg-gray-950">
                <div class="pt-5">
                            ...
                            <div
                                ...
                                @forelse ($kampus->Gambar as $gambar)
                                    <img class="group-hover:scale-110 absolute w-full h-full transition-all duration-700 bg-cover"
                                        alt="Card Fatir" src="{{ asset('storage/' . $gambar->gambar) }}" />
                                @empty
    ...
    @endif
    <!-- daftar semua kampus -->
    <section class="dark:bg-gray-950 pt-11 lg:pt-14 pb-5 min-h-[760px] ">
        <div class="container">
            <div class="mt-5 mb-1">
            ...
            <div class="md:grid-cols-2 lg:grid-cols-2 grid grid-cols-1 gap-2 p-2 md:gap-4">
                @forelse ($kampuses as $kampus)
                    <div class="card card-compact lg:card-side bg-base-100 dark:bg-gray-950 shadow-xl">
                        @forelse ($kampus->Gambar as $gambar)
                            <figure class="h-15 aspect-[3/2] rounded-lg md:aspect-[6/4] md:flex overflow-hidden lg:aspect-video lg:min-w-[250px]">
                                <img class="rounded-lg w-full h-full" src="{{asset('storage/'. $gambar->gambar)}}" alt="Kampus"/>
                            </figure>
                        @empty
